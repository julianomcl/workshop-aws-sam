...
- **`calculator`** - Code for the application's Lambda function.
...
```bash
calculator-powertools$ make build
calculator-powertools$ make deploy.guided
```
...
The SAM CLI installs dependencies defined in `calculator/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.

Test a single function by invoking it directly with a test event:

```bash
calculator-powertools$ make invoke
```
...
```bash
calculator-powertools$ make run
calculator-powertools$ curl http://localhost:3000/hello
```
...
```bash
calculator-powertools$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
calculator-powertools$ make dev
calculator-powertools$ make test
```
...
```bash
calculator-powertools$ aws cloudformation delete-stack --stack-name calculator-powertools
```
...
* **`make invoke`**: `sam local invoke HelloWorldFunction --event events/calculator_event.json`
...
```bash
calculator-powertools$ pipenv lock -r > calculator/requirements.txt
```
